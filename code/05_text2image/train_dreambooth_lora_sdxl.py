...
def main(args):
    ...
    if torch.backends.mps.is_available() and args.mixed_precision == "bf16":
        # MPS does not support bfloat16 yet
        raise ValueError(
    ...
    # Accelerator and distributed training setup
    ...
    # Disable automatic mixed-precision (AMP) on MPS
    ...
    # If selected, set the training seed
    ...
    # Create the output folder and the HF repository
    ...
    # Define the correct text encoder classes
    ...
    # For mixed-precision training, we cast all non-trainable weights (VAE, non-LoRA text encoder 
    # and non-LoRA U-Net) to half-precision as these weights are only used for inference, 
    ...
    # Move U-Net, VAE and text encoders to computing device and cast weights to weight_dtype
    ...
    # ------------------------------------------------------------------------------
    # Custom saving hook that allows `accelerator.save_state(...)` serialize weights in adequate format
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            # there are only two options here. Either are just the U-Net attention processor layers
            # or there are the U-Net and text encoder attention layers
    ...
    # ------------------------------------------------------------------------------
    # Custom loading hook that allows `accelerator.save_state(...)` deserialize weights using adequate format
    ...
    # Computes additional embeddings/ids required by the SDXL U-Net.
    ...
    # Handle class prompt for prior-preservation loss
    if args.with_prior_preservation:
        if not args.train_text_encoder:
    ...
    # Free memory occupied by tokenizers and text encoders
    if not args.train_text_encoder and not train_dataset.custom_instance_prompts:
    ...
    # pack the statically computed variables appropriately here. This is so that we do not
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        # if we are optimizing the text encoder (both if instance prompt is used for all images or custom prompts) 
    ...
    # Setup the learning rate scheduler. We need to recalculate our number of training steps 
    # as the size of the training dataloader may have changed after we prepare it with 
    # `accelerator.prepare`.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Predict the noise that must be removed from the latents
                if not args.train_text_encoder:
