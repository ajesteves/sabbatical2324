...
def get_celeba_config():
  ...
  experiment.mode                = "train"   # "train" or "sampling" or "summary"
  ...
  training.p_uncond              = 0.1       # Probability of training the unconditional generation.
  ...
  sampling.batch_size            = 5600      # Batch size during the sampling process.
  ...
  sampling.saved_model           = None      # File (NOT path) containing the model to use during sampling.
...
# ==============================================================================
# Class/Timestep Embedding Layer
# ==============================================================================

class ConditionalEmbedding(nn.Module):
    '''
    Embedding layer for conditioning class/timestep 'c'.
    '''
...
def load_data(
    ...
    # use 'shuffle=True' in DistributedSample, not in DataLoader
        ...
        for epc in range(self.epoch, self.config.training.epochs):
            ...
            with tqdm(dataloader, dynamic_ncols=True, disable=(self.local_rank % self.world_size != 0)) as tqdmloader:
                for img, label in tqdmloader:
                    ...
                    cemb[np.where(np.random.rand(b)<self.config.training.p_uncond)] = 0
...
#   torchrun --standalone --nproc_per_node=2 CFreeGuidance_01_exp02.py
