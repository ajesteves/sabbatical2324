'''
Apply pipeline parallelism to train a simple transformer decoder model.
'''
# --------------------------------------------------------------------
import os
import time
...
from   torch.distributed.pipelining import (
       pipeline, SplitPoint, PipelineStage, ScheduleGPipe
       )
...
class Transformer(nn.Module):
   def __init__(self, model_args: ModelArgs):
      ...
      # ensuring checkpoints can be saved and loaded correctly
   ...
   def forward(self, tokens: torch.Tensor):
      # Handling layers being 'None' at runtime enables splitting the pipeline easily
...
def manual_model_split(model) -> PipelineStage:
   if stage_index == 0:
      # Prepare the first model's stage with layers 0:3
      for i in range(4, 8):
      ...
      # Prepare the second model's stage with layers 4:7
      for i in range(4):
            del model.layers[str(i)]
...
if __name__ == "__main__":
   ...
   # 1 - split the model manually
   stage = manual_model_split(model)

   # 2 - split the model with the help of the tracer
   ...
   for id in range(10000):
      ...
      if rank == 0:
         ...
         if id % 100 == 0:
            print(f"[step {id}] loss from microbatches is {[f'{l.item():.4f}' for l in losses]}")

   if rank == 0:
       te = time.time()
       print(f"Total time: {te - ts} s")
