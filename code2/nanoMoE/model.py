...
class LayerNorm(nn.Module):
    ...
    PyTorch does not support simply setting bias=False.
    """
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # output projection
        self.fc2 = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
    ...
    def forward(self, x):
        ...
        # output projection
        y = self.resid_dropout(self.fc2(y))
        return y
...
class MLPExperts(nn.Module):
    """
    Implementation of multiple MLP-based experts that can process inputs
    in batches. It is based upon ColossalAI OpenMoE but simpler, has optional bias,
    and uses a batched matrix multiplication, instead of a series of matrix 
    multiplications for each expert, to improve efficiency.
    https://github.com/hpcaitech/ColossalAI/blob/main/colossalai/moe/experts.py
    """
    def __init__(self, config):
        # TODO: add param init
        super().__init__()
        ...
        self.fc1         = nn.Parameter(torch.empty(config.n_exp, config.n_embd, 4 * config.n_embd))
        self.fc1_bias    = nn.Parameter(torch.empty(config.n_exp, 1, 4 * config.n_embd)) if self.bias else None
        self.fc2         = nn.Parameter(torch.empty(config.n_exp, 4 * config.n_embd, config.n_embd))
        self.fc2_bias    = nn.Parameter(torch.empty(config.n_exp, 1, config.n_embd)) if self.bias else None
        self.gelu        = nn.GELU()
        self.dropout     = nn.Dropout(config.dropout)
...
@dataclass
class MOEconfig:
    # Dense transformer model parameters
    block_size: int   = 1024
    vocab_size: int   = 50304 # vocabulary size of 50257, padded up to nearest 
                              # multiple of 64 for efficiency
    n_layer:    int   = 12
    n_head:     int   = 12
    n_embd:     int   = 768
    dropout:    float = 0.0
    bias:       bool  = True # True: bias in Linear and LayerNorm layers, 
                             # like GPT-2. False: a bit better and faster
...
class Transformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.n_exp == 1:
            ...
            for i in range(config.n_layer):
                # TODO: should we change 'i' to 'i+1' in next line?
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('fc2.weight') or pn.endswith('experts.fc2'):
                # print(f'====> Initializing {pn} with normal_') ############ DEBUG CODE ##########
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if self.config.use_switch_tfm_init:
                ...
                fc1_fan_in = module.fc1.shape[-2]
                fc1_std    = (scale / fc1_fan_in) ** 0.5
                torch.nn.init.trunc_normal_(
                    module.fc1,
                    mean = 0.0,
                    std  = fc1_std,
                ...
                fc2_fan_in = module.fc2.shape[-2]
                fc2_std    = (scale / fc2_fan_in) ** 0.5
                torch.nn.init.trunc_normal_(
                    module.fc2,
                    mean = 0.0,
                    std  = fc2_std,
                ...
                # perform standard (normal) initialization of weights
                torch.nn.init.normal_(module.fc1, mean=0.0, std=0.02)
                torch.nn.init.normal_(module.fc2, mean=0.0, std=0.02)
    ...
    def from_pretrained(cls, model_type, override_args=None):
        ...
        print("[INFO] Loading weights from pretrained gpt %s" % model_type)
        ...
        print("[WARN] Forcing vocab_size=50257, block_size=1024, bias=True")
        ...
        if 'dropout' in override_args:
            print(f"[INFO] Overriding dropout rate to {override_args['dropout']}")
            config_args['dropout'] = override_args['dropout']

        # create a from-scratch initialized minGPT model
        config  = MOEconfig(**config_args)
        ...
        # copy while ensuring that all parameters are aligned and match in names and shapes
        sd_keys_hf = sd_hf.keys()
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.masked_bias')] # ignore these, just a buffer
        sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.bias')] # same, just the mask (buffer)
        transposed = ['attn.c_attn.weight', 'attn.fc2.weight', 'mlp.fc1.weight', 'mlp.fc2.weight']
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        ...
        print(f"[INFO] Number of decayed parameters: {len(decay_params)}, with {num_decay_params:,} parameters")
        print(f"[INFO] Number of non-decayed parameters: {len(nodecay_params)}, with {num_nodecay_params:,} parameters")
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        """
        Take a conditioning sequence of indices idx, a LongTensor of shape (b,t), and complete
        the sequence max_new_tokens times, feeding the predictions back into the model each time.
        Most likely we have to be sure that the model is in model.eval() mode of operation for this.
        """
